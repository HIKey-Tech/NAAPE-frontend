...
// Shared between the public /trainings/[id] page and the member portal /training/[id] page.
// shareBase controls which URL the Share button copies (public link is always shareable).
export default function TrainingDetail({ id }: { id?: string }) {
    ...
    const handleShare = async () => {
        // Always share the public URL so recipients don't hit the auth wall
        const url = `${window.location.origin}/trainings/${id}`;
        if (navigator.share) {
            try {
    ...
    if (isError || !training) {
        return (
            <div className="flex justify-center items-center py-32 text-center px-4">
                <div>
