...
export interface PublicationTableProps {
  ...
  renderAction?: (publication: Publication) => ReactNode;
}

const statusInfo: Record<Publication["status"], { label: string; className: string; dotColor: string }> = {
  Pending: {
    label: "Pending",
          ...
          {publications.length === 0 ? (
            ...
            publications.map((pub) => (
              <tr key={pub.id} className="hover:bg-gray-50 transition-colors">
                ...
                {/* Submitted */}
                <td className="px-4 py-3 text-gray-700 whitespace-nowrap" title={pub.submitted}>
                  {formatDate(pub.submitted)}
                </td>
                {/* Status */}
