    ...
    return (
        ...
        >
            <div className="max-w-4xl w-full flex flex-col items-center text-center gap-4 mb-2">
                <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-slate-900 leading-tight mb-1 tracking-tight" style={{ letterSpacing: "-.025em" }}>
                    <span className="inline-block border-b-4 border-primary pb-[0.18em] px-3 bg-primary/5 rounded-md shadow-none">{heading}</span>
                </h1>
                {subheading && (
                    <p className="text-primary text-lg md:text-xl font-semibold max-w-2xl mx-auto mt-1">{subheading}</p>
                )}
                {children}
            </div>
            <div className="w-full flex justify-center mt-4">
                <div
                    className="relative rounded-2xl h-72 md:h-96 overflow-hidden w-full max-w-3xl border-2 border-primary bg-slate-50 group transition-all duration-200"
                    ...
                    {showArrows && hasMultipleSlides && (
                        <>
                            <button
                                ...
                                className="absolute left-2 top-1/2 -translate-y-1/2 z-20 bg-primary/10 hover:bg-primary/20 border-2 border-primary text-primary rounded-full p-2 transition-colors focus:outline-none focus:ring-2 focus:ring-primary/30"
                                onClick={goPrev}
                                tabIndex={0}
                            ...
                            <button
                                ...
                                className="absolute right-2 top-1/2 -translate-y-1/2 z-20 bg-primary/10 hover:bg-primary/20 border-2 border-primary text-primary rounded-full p-2 transition-colors focus:outline-none focus:ring-2 focus:ring-primary/30"
                                onClick={goNext}
                                tabIndex={0}
                                style={{ visibility: total < 2 ? "hidden" : undefined }}
                            >
                                ...
                                {(slides[current].caption || slides[current].headline || slides[current].excerpt) && (
                                    ...
                                    >
                                        {slides[current].headline && (
                                            <span className="block text-slate-900 text-xl md:text-2xl font-black mb-1 leading-snug transition-colors">
                                                {slides[current].link ? (
                                        ...
                                        {slides[current].excerpt && (
                                            <span className="block text-primary text-base md:text-lg font-semibold mt-1">{slides[current].excerpt}</span>
                                        )}
                                        {slides[current].caption && (
                                            <span className="block text-slate-400 text-xs mt-2 font-medium">{slides[current].caption}</span>
                                        )}
                                    </motion.div>
                                )}
                            </motion.div>
                        ...
                        <div className="absolute bottom-4 left-6 flex gap-2 z-20" aria-label="Slide indicators">
                            {slides.map((_, idx) => (
                                <button
                                    ...
                                    className={`w-3.5 h-3.5 rounded-full border-2 border-primary transition-all duration-300 focus:ring-2 focus:ring-primary/30 outline-none
                                        ${idx === current
                                            ? "scale-125 bg-primary"
                                            : "bg-white hover:bg-primary/20/70"}
                                    `}
