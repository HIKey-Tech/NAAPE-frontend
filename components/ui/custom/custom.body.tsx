...
export default function CustomBodySection({
  title = (
    <>
      The Birth of NAAPE
  ...
  imageAlt = "Pilots in the cockpit of an airplane",
  containerClassName = "flex flex-col md:flex-row items-start md:items-center gap-12 w-full py-16 px-4 md:px-16 bg-white",
  textClassName = "flex-1 max-w-2xl",
  imageClassName = "rounded-2xl transition-transform duration-300 hover:scale-105",
}: CustomBodySectionProps) {
  return (
    <motion.section
    ...
    >
      ...
      >
        ...
        >
          {paragraphs && paragraphs.map((para, idx) => <p key={idx}>{para}</p>)}
        </motion.div>
      </motion.div>
      {/* Image Content */}
      <motion.div
