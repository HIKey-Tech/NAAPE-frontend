...
export default function NaapePublicationDetailPage() {
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] py-10 px-2 sm:px-0 flex flex-col items-center">
      <PublicationDetail
        imageUrl={publication.image || "/logo.png"}
        title={publication.title}
        summary={publication.summary}
        content={publication.content}
        authorName={publication.author?.name || "NAAPE"}
        authorRole={publication.author?.role || "Admin"}
        authorAvatarUrl="/logo.png"
        category={publication.category}
        publishedDate={publication.createdAt}
        backHref="/publication/naape"
      />
      
      {/* Premium Access Prompt - NAAPE publications might be free for all users */}
      {publication.isPreview && publication.requiresSubscription && (
        <div className="w-full max-w-3xl mx-auto mt-8 bg-white border border-slate-200 rounded-2xl p-8 text-center shadow-sm">
          ...
          <p className="text-slate-600 mb-6">
            This is a preview of the full publication. Sign in or subscribe to read the complete article and access our full library of publications.
          </p>
